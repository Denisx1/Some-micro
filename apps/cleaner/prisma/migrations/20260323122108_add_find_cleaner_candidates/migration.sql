...
RETURNS TABLE (id INT, user_id INT, first_name TEXT, last_name TEXT, experience_years INT, rating INT, created_at TEXT, start_time TEXT, end_time TEXT)
AS $$
BEGIN
    RETURN QUERY
    SELECT p.id, 
        p."userId",
        p."firstName"::TEXT,      -- Добавил каст к TEXT для соответствия RETURNS TABLE
        p."lastName"::TEXT,
        p."experienceYears"::INT,
        p."rating"::INT,
        p."createdAt"::TEXT,      -- Добавил запятую (была пропущена)
        s."startTime"::TEXT,      -- Добавил каст к TEXT
        s."endTime"::TEXT
